  ...
  return (
    <div className="">
      <div className="flex justify-center  w-full">
        <div key={blogDetails.id} className="flex flex-col w-[60%] items-center gap-[60px]">
          ...
          <div className="">
            {BlogThumbnail && (
              <img className="w-[100%] rounded-[14px]"
                src={`http://localhost:8082${BlogThumbnail.data.attributes.url}`}
                alt={BlogThumbnail.data.attributes.name}
              />
            )}
          </div>
          <StyledDes dangerouslySetInnerHTML={renderParagraphs(BlogDescription)} />
        </div>
      </div>
    </div>
  );
